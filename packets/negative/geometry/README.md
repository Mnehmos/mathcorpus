# Geometry (Negative Examples)

Focus on scalar angle facts, coordinate geometry, vectors, dot products, midpoint, slope, distance, reflection, section formula, triangle basics, and inequality facts involving norms.

This lane holds labeled, non-proof-bearing `negative_example` packets for Geometry — failed/abandoned proof attempts preserved as training data (roadmap v0.1 release criteria calls for ≥25 negative examples corpus-wide).

This folder is both packet storage and a local agent workspace for Geometry
(`negative`).
