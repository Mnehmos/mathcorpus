# Loop — Inequalities (Negative Examples)

## Agent role

You are the local MathCorpus agent for `packets/negative/inequalities`. Your job is to add verified
...
3. If a route fails, don't discard it — turn it into a `negative_example`
   ...
   `trust.rung: 0`) under `packets/negative/inequalities/` and note it in
   `BLOCKERS.md`.
