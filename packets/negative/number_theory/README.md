# Number Theory (Negative Examples)

Focus on divisibility, gcd, primes, modular arithmetic, parity, factorials, powers, induction over natural numbers, and small reusable lemmas. Prefer clean statements that teach proof structure.

This lane holds labeled, non-proof-bearing `negative_example` packets for Number Theory — failed/abandoned proof attempts preserved as training data (roadmap v0.1 release criteria calls for ≥25 negative examples corpus-wide).

This folder is both packet storage and a local agent workspace for Number Theory
(`negative`).
