# Packet Template — Erdős Frontier
...
Anti-overclaim note: every open-problem-adjacent packet must explicitly
state (in `notes`) that it does **not** solve the open problem, and must
clearly separate what it proves (a companion/partial result) from the open
conjecture itself.
