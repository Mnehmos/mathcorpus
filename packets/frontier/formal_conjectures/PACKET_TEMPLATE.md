# Packet Template — Formal Conjectures
...
Frontier note: a Formal Conjectures packet starts life as a statement-only
artifact (no completed `proof_body_path` beyond a `sorry` placeholder,
`status: formula_fact_only` or similar). It is only promoted to a proof
packet once Lean-verified through a tracked episode, and only promoted to
training-eligible once `STATEMENT_FIDELITY.md` is satisfied.
