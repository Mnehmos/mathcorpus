# Validation — Algebra (Elementary)

Before marking a packet complete in this domain, run (from repo root):

```bash
python tools/stamp_hashes.py packets/elementary/algebra/<name>.v<version>.json
python tools/validate_packets.py packets/elementary/algebra/<name>.v<version>.json --check-hashes --warn-as-error
```
